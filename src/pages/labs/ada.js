...
const generateSVG = (rows, color) => {
  ...
  for (let y = 0; y < l; y++) {
    ...
    svg += `<circle cx="${
      24.95 + r
    }" cy="${cy}" r="${rl}" stroke="black" stroke-width="${sw}" style="fill:none" fill="transparent"  />`
  ...
  svg += `
  <rect x="13" y="3" width="141" height="${
    dy * l + oy
  }" fill="transparent" style="fill:none" stroke="black" stroke-width="${sw}" />
  </svg>`
  return svg
}
      ...
      <Layout location={location} bodyClass="ada-algorithm">
        ...
        <p>
          <Trans i18nKey="elementary-cellular-automaton">
            ...
            <a
              ...
              target="_blank"
            >
              elementary elementary cellular automaton
            </a>
            <span>
              {' '}
              where the rule to determine the state of a cell in the next
              generation depends only on its current state and its two immediate
              neighbors.
            </span>
          </Trans>
        </p>
        <p>
          <Trans i18nKey="algorithm-explanation">
            We start from a random state, and then cycle through a selection of
            rules every <strong>24</strong> rows.
          </Trans>
        </p>

...
const Connection = () =>{
  ...
  return (
    <div className="connection">
      ...
      <p>If there is an active live emojing session, you can <strong>watch it</strong> or <strong>join it</strong> and be part of the session. Otherwise, you can just <strong>play alone</strong>.</p>
      <Avatar/>
      <Nick/>
      {context.connected ?
        (context.servers.length===0 ?
          <p><Emoji emoji="😢" /> Sadly, no live emojing sesison was found...<em>You dont even need a nickname</em></p>
          :(<>
            {t('liveCodingSessions', {count:context.servers.length})}
            <p>When joining a session, be sure to be also watching it on another window or device. {`Does'nt`} make much sense to only send your emojis.</p>
      ...
      : <p> <Emoji emoji="😭" /> Cannot get session list.</p>}

...
class YM2612Generator extends AudioWorkletProcessor {
  ...
  process(inputs, outputs, parameters) {
    ...
    const buffer = this.ym.update(128)

    for (let channel = 0; channel < output.length; ++channel) {
      let outputChannel = outputs[0][channel];
      for (let i = 0; i < outputChannel.length; ++i) {
        outputChannel[i] = buffer[channel][i]/32768
      }
    }
...
registerProcessor('ym2612-generator', YM2612Generator)
